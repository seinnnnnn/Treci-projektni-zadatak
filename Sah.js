class Game {
	...
	unblockedPositions(allowedPositions=[], position, color, checking=true){
		...
		if (this.clickedPiece.hasRank('pawn')) {
			for (const move of allowedPositions[0]) { //Šahovi
			...
			for (const move of allowedPositions[1]) { //pomjeranje figura
			...
			if (clickedPiece) {
				...
				if (this.king_checked(this.turn)) {
					if (this.king_dead(this.turn)) {
						...
						// Upozorenje kad je kralj pod šahom;
					}
				}
			}
			else{
	...
	checkmate(color){
		...
		endScene.getElementsByClassName('winning-sign')[0].innerHTML = color + ' je pobjedio';
